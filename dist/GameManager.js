    ...
    addHandler(socket) {
        ...
        socket.on("message", (data) => {
            ...
            if (message.type === INIT_GAME) {
                ...
                const matchingUser = this.pendingUsers.find((u) => u.timeControl === userTimeControl);
                if (matchingUser) {
                    console.log(`Starting Game - ${matchingUser.username} vs ${currentUsername}`);
                    // 👤 CREATE GAME WITH USERNAMES
                    const game = new Game(matchingUser.socket, socket, userTimeControl, matchingUser.username, currentUsername);
                    this.games.push(game);
                    // Remove matched user from pending
                    this.pendingUsers = this.pendingUsers.filter((u) => u.socket !== matchingUser.socket);
                }
                else {
                    console.log(`${currentUsername} waiting for opponent with same time control...`);
